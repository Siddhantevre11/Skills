...
This is where I stop the agent from defaulting to horizontal layers (all frontend, then all backend, then tests). The vertical-slice + "blocked by" structure forces a dependency-ordered kanban board, so the agent works one complete, demoable slice at a time instead of a half-built system in every layer.